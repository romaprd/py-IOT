...
void loop() {
  // Controle do sensor PIR
  if (digitalRead(pinoPIR) == HIGH) {  // Se o sensor PIR detectar movimento (HIGH)
    digitalWrite(LED, HIGH); // Acende o LED
    Serial.println("Movimento detectado! LED aceso.");
  } else {  // Se não houver movimento (LOW)
  ...
  if (millis() - lastLcdUpdate >= lcdInterval) {
    ...
    // Exibir o status do sensor PIR no LCD
    lcd.setCursor(0, 0);
    lcd.print("Sensor PIR:");
    if (digitalRead(pinoPIR) == HIGH) {  // Se o PIR detectar movimento
      lcd.setCursor(0, 1);
      lcd.print("Movimento! ");
